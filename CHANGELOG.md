...
- **In-TUI Playback Controls (Stop / Pause)**:
  ...
  - Pressing `q` while a stream is playing now stops playback and keeps the app open; press `q` again to quit. `Ctrl+C` and `/exit` behave the same while playing.
  - The Home bottom bar, the Details footer, and the in-app help menu surface the active playback controls and the current paused state.
  - Android external-player limitation is surfaced explicitly: playback delegated to an external Android app cannot be paused/stopped from within MovieBox-TUI; `x` only resets the in-app session while the video app keeps playing.
- **VLC Plays Header-Protected Streams via a Loopback Relay**:
  - Added a tiny loopback HTTP relay (`127.0.0.1`, ephemeral port) that fronts a playback URL for the duration of a session. When a stream needs headers VLC's CLI cannot express (notably MovieBox's signed CloudFront `Cookie`), MovieBox-TUI launches VLC against the relay URL; the relay re-issues every manifest and relative-segment request against the original origin with the full header set attached and streams responses back byte-for-byte (byte ranges and multi-quality DASH preserved).
  - `supports_headers` now reports VLC as end-to-end header-capable, so a MovieBox stream resolves to VLC instead of the previous "VLC lacks header support" warning; the relay is torn down when the player exits or fails to spawn.
  - Android intent playback remains limited to `Referer`/`User-Agent` (and only via `termux-am`/`am`), with `mpv`/`VLC` guidance for cookie-protected MovieBox streams.

### Changed
- **Opener-Aware Android Playback**:
  - `probe_android_opener` now prefers `termux-am`/`am` over `termux-open`/`termux-open-url` on both `$PREFIX` and static Termux paths, so dispatched streams always travel through the intent tool that can set the `video/*` MIME type, forward `User-Agent`/`Referer` and subtitle extras, and target a specific package.
  - The `supports_headers` gate for Android is now opener-aware: `Referer`/`User-Agent` streams are only dispatched through `termux-am`/`am`; when only `termux-open`/`termux-open-url` is available they are rejected with a clear warning instead of launching with required headers silently dropped. Signed `Cookie` streams remain mpv-only.
  - Added `MOVIEBOX_ANDROID_PACKAGE` for pinning the receiving Android app (`-p <pkg>` on am paths, second positional argument to `termux-open-url`), letting playback skip the resolver/chooser and open a specific player (VLC, Just Player, MPV Android, MX Player, …).
  - Replaced Termux "no player" messages with action-oriented text ("Could not find an Android video player. … then run: pkg install -y termux-tools termux-am"), added an explicit "This stream requires HTTP headers that cannot be passed through the Android player." warning for Android header rejection, and an "Unable to open the stream with an Android video player: …" spawn-failure message.
  - Updated player/playback unit tests for the opener-aware contract and documented detect order, capabilities, and limitations in `README.md` and `docs/players.md`.
