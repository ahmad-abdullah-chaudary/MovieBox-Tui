...
| `MOVIEBOX_PLAYER`         | Preferred player (overrides `default_player`): `mpv`, `iina`, `vlc`, or `android`. |
...
| `MOVIEBOX_ANDROID_PLAYER_PATH` | Custom Android intent tool. Recognizes basenames `termux-am`/`am`, `termux-open-url`, and `termux-open` to determine intent capabilities. |
| `MOVIEBOX_ANDROID_PACKAGE`    | Pin the Android app that receives `ACTION_VIEW` (e.g. `org.videolan.vlc`, `com.brouken.player`, `is.xyz.mpv`, `com.mxtech.videoplayer.ad`). Sent via `-p <pkg>` on `am` paths or a second positional argument to `termux-open-url`, skipping the resolver/chooser. |
