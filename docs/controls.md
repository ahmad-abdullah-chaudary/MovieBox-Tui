...
| **`Space` / `P`** | Direct resume playback for recorded season/episode on `/history` or Home Resume items; while a stream is playing, pause / resume the player |
| **`x` / `X`** | Stop the active playback (unless a download is in progress, where it cancels the download instead) |
...
| **`Ctrl+C` / `q`** | Quit application and restore terminal. While a stream is playing, the first press stops playback instead — press again to quit |

## Playback Controls

While a stream is playing, the Home bottom bar and the Details footer display the active controls:

| Key | Action |
| :--- | :--- |
| **`Space` / `P`** | Pause or resume the running player (local `mpv`/`VLC`/`IINA` child processes). |
| **`x` / `X`** | Stop playback and return to browsing without leaving the app. |
| **`q` / `Ctrl+C`** | First press stops playback; a second press quits MovieBox-TUI. |

Pause/stop act on the process MovieBox-TUI spawned. Android playback is delegated to an external Android video app, so it cannot be paused or stopped from the terminal — use the video app's own controls; `x` only clears the in-app playback session. On Windows the player cannot be suspended, so pause is unavailable there.
