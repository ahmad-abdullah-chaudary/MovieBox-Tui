...
#[tokio::test]
async fn test_media_player_picker_choice_is_rendered_as_default() {
    let mut app = App::new();
    app.state_mut().available_players =
        vec![PlayerKind::Mpv, PlayerKind::Vlc, PlayerKind::AndroidIntent];
    app.state_mut().default_player = Some("mpv".to_string());
    app.state_mut().settings_category = SettingsCategory::General;
    app.handle_action(Action::ShowSettingsPopup).await;
    app.handle_action(Action::Key(KeyEvent::new(
        KeyCode::Down,
        KeyModifiers::empty(),
    ...
    let before = render_app(&mut app);
    assert!(before.contains("mpv"), "initial row should show mpv");

    app.handle_action(Action::SettingsActivateRow).await;
    assert!(app.state().player_picker_popup);
    while app.state().player_picker_state.selected() != Some(2) {
        app.handle_action(Action::MoveDown).await;
    }
    app.handle_action(Action::Submit).await;
    assert!(!app.state().player_picker_popup);
    assert_eq!(app.state().default_player.as_deref(), Some("android"));

    let after = render_app(&mut app);
    assert!(
        after.contains("Android Player"),
        "settings modal should show the chosen player as default; got: {}",
        after
    );
}
