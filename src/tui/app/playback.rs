...
use crate::tui::{action::Action, overlay::NotificationKind, state::PlayerKind, state::Screen};

const ANDROID_HEADER_LIMITATION: &str =
    "This stream requires HTTP headers that cannot be passed through the Android player.";
...
impl App {
    ...
    pub(super) fn dispatch_playback_or_notify(
        ...
        match self.resolve_playback_player(&source) {
            ...
            PlaybackResolution::ExplicitPlayerIncompatible {
                ...
                if chosen == PlayerKind::AndroidIntent {
                    let body = if compatible_alternatives.is_empty() {
                        ANDROID_HEADER_LIMITATION.to_string()
                    } else {
                        let alternatives_str = compatible_alternatives
                            ...
                            .join(" or ");
                        format!(
                            "{ANDROID_HEADER_LIMITATION} Switch to {alternatives_str} in /settings."
                        )
                    };
                    self.state.notify(
                        NotificationKind::Warning,
                        "Android Player Incompatible",
                        body,
                    );
                } else {
                    let body = if !compatible_alternatives.is_empty() {
            ...
            PlaybackResolution::NoCompatiblePlayer { available: _ } => {
                ...
                let supported_str = crate::player::header_capable_players()
                    ...
                    .collect::<Vec<_>>()
                    .join(" or ");
                let body = if crate::updater::artifact::is_termux_environment() {
                    format!(
                        "{ANDROID_HEADER_LIMITATION} Install a header-capable player or pick a stream that does not require headers."
                    )
                } else {
                    format!("Headers unsupported. Install {supported_str}.")
                };
                self.state
                    .notify(NotificationKind::Error, "Incompatible Media Player", body);
            }
            PlaybackResolution::NoPlayersInstalled => {
                self.state.is_resolving_playback = false;
                self.state.pending_playback_source = None;
                let message = if crate::updater::artifact::is_termux_environment() {
                    "Could not find an Android video player. Install VLC, Just Player, MPV Android, or another player that supports video playback, then run: pkg install -y termux-tools termux-am"
                } else {
    ...
    pub(super) fn launch_player(
        ...
        self.state.is_resolving_playback = false;
        self.state.player_paused = false;
        self.state.is_stopping_playback = false;
        self.active_player_pid = None;
        ...
        tokio::spawn(async move {
            ...
            let relay = if crate::player::needs_http_relay(kind, &headers) {
                match crate::player::relay::start(&link, headers.clone()).await {
                    Ok(relay) => {
                        log::info!(
                            "started loopback HTTP relay for {kind:?} at {}",
                            relay.base_url
                        );
                        Some(relay)
                    }
                    Err(error) => {
                        log::error!("failed to start loopback HTTP relay for {kind:?}: {error}");
                        sender
                            .send(Action::PlayerCrashed(
                                None,
                                format!(
                                    "Failed to launch {}. Could not start the local stream relay: {error}",
                                    kind.label()
                                ),
                            ))
                            .ok();
            ...
            let play_url = relay
                .as_ref()
                .map(|relay| relay.play_url.clone())
            ...
            match command.spawn() {
                Ok(mut child) => {
                    ...
                    sender.send(Action::PlayerStarted { pid: spawned_pid }).ok();
                ...
                Err(error) => {
                    ...
                    let message = if kind == crate::tui::state::PlayerKind::AndroidIntent {
                        format!("Unable to open the stream with an Android video player: {error}")
                    } else {
                        format!("Failed to spawn player executable: {error}")
                    };
                    sender.send(Action::PlayerCrashed(None, message)).ok();
                    sender.send(Action::PlayerExited).ok();
                }
            }
        });
...
impl App {
        ...
        match action {
            ...
            Action::LaunchPlayback(kind, source) => {
                ...
                if !crate::tui::player::supports_headers(kind, &source.headers) {
                    let (title, body) = if kind == PlayerKind::AndroidIntent {
                        (
                            "Android Player Incompatible".to_string(),
                            ANDROID_HEADER_LIMITATION.to_string(),
                        )
                    } else {
                        (
            ...
            Action::PlayerStarted { pid } => {
                self.active_player_pid = pid;
            }
            Action::StopPlayback => {
                if self.stop_playback_if_active() {
                    self.state
                        .set_status_short("Playback stopped. Press q again to quit the app.");
                } else {
                    self.state.set_status_short("No active playback to stop.");
                }
            }
            Action::TogglePlayerPause => {
                self.toggle_player_pause();
            }
            Action::PlayerCrashed(code, error_msg) => {
                if self.state.is_stopping_playback {
                    self.state.is_stopping_playback = false;
                    self.state.is_playing = false;
                    self.state.is_resolving_playback = false;
                    self.state.player_paused = false;
                ...
                let (title, message) = if is_termux_perm_crash {
                    (
                        ...
                        "Could not find an Android video player. Install VLC, Just Player, MPV Android, or another player that supports video playback, then run: pkg install -y termux-tools termux-am".to_string(),
                    )
                } else {
    ...
    pub fn stop_playback_if_active(&mut self) -> bool {
        if let Some(pid) = self.active_player_pid.take() {
            self.state.is_stopping_playback = true;
            terminate_player_process(pid);
            return true;
        }
        if self.state.is_playing {
            self.state.is_playing = false;
    ...
    fn toggle_player_pause(&mut self) {
        if !self.state.is_playing {
            self.state.set_status_short("No active playback to pause.");
            return;
        }
        let Some(pid) = self.active_player_pid else {
            self.state.notify(
                NotificationKind::Warning,
                "Pause unavailable",
                "Playback runs in an external app (or the player session ended) and cannot be paused from here.",
            );
            return;
        };
        #[cfg(unix)]
        {
            let sig = if self.state.player_paused {
                libc::SIGCONT
            } else {
                libc::SIGSTOP
            };
            ...
            if rc == 0 {
                self.state.player_paused = !self.state.player_paused;
                if self.state.player_paused {
                    self.state
                        .set_status_short("Player paused. Press Space to resume.");
                } else {
                    self.state.set_status_short("Playback resumed.");
                }
            } else {
                self.active_player_pid = None;
                self.state.is_playing = false;
                self.state.player_paused = false;
                self.state
                    .set_status_short("Player session ended; nothing to pause.");
            }
        }
        #[cfg(not(unix))]
        {
            ...
            self.state.notify(
                NotificationKind::Warning,
                "Pause unavailable",
                "Pause requires a POSIX terminal (Termux / Linux / macOS). On Windows the player cannot be suspended from inside MovieBox-TUI.",
            );
        }
    }
}

fn terminate_player_process(pid: u32) {
    #[cfg(unix)]
    {
        unsafe {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_vlc_is_compatible_with_cookie_streams_via_the_relay() {
        let mut app = crate::tui::app::App::new();
        ...
        assert!(crate::player::needs_http_relay(
            crate::tui::state::PlayerKind::Vlc,
            &source.headers
        ));
        assert_eq!(
    ...
    async fn test_explicit_player_incompatible_does_not_launch_alternative() {
        ...
        assert!(
            notification
                .message
                .contains("Switch to mpv or VLC in /settings.")
        );
    }
    #[tokio::test]
    ...
    #[tokio::test]
    async fn test_android_player_header_capability_is_opener_aware() {
        let mut app = crate::tui::app::App::new();
        app.state.available_players = vec![crate::tui::state::PlayerKind::AndroidIntent];
        app.state.default_player = Some("android".to_string());
        ...
        // On a host without an intent-capable opener (termux-am / am), Referer and
        // User-Agent streams must not be silently dispatched to the Android player:
        // the resolved opener cannot forward their required headers.
        ...
        app.state.is_resolving_playback = true;
        app.dispatch_playback_or_notify(fourk_source);
        assert!(!app.state.is_resolving_playback);
        let notification = app.state.notifications.back().expect("notification posted");
        assert_eq!(notification.title, "Android Player Incompatible");
    ...
    #[tokio::test]
    async fn test_stop_playback_ends_in_app_session_without_pid() {
        let mut app = crate::tui::app::App::new();
        app.state.is_playing = true;
        app.state.is_resolving_playback = false;
        app.state.player_paused = true;

        assert!(app.stop_playback_if_active());
        assert!(!app.state.is_playing);
        assert!(!app.state.player_paused);
        assert_eq!(app.active_player_pid, None);
    }

    #[tokio::test]
    async fn test_stop_playback_tracks_requested_stop_for_live_player() {
        let mut app = crate::tui::app::App::new();
        app.state.is_playing = true;
        app.active_player_pid = Some(61_000_000);

        assert!(app.stop_playback_if_active());
        assert!(app.state.is_stopping_playback);
        assert_eq!(app.active_player_pid, None);

        app.handle_action(Action::PlayerCrashed(Some(1), "test".to_string()))
            .await;
        assert!(!app.state.is_playing);
        assert!(!app.state.is_stopping_playback);
    }
    ...
    #[tokio::test]
    async fn test_player_exited_clears_pause_pid_and_stop_flag() {
        let mut app = crate::tui::app::App::new();
        app.state.is_playing = true;
        app.state.player_paused = true;
        app.state.is_stopping_playback = true;
        app.active_player_pid = Some(7000);
    ...
    #[tokio::test]
    async fn test_player_started_records_pid() {
        let mut app = crate::tui::app::App::new();
        app.handle_action(Action::PlayerStarted { pid: Some(4242) })
            .await;
        assert_eq!(app.active_player_pid, Some(4242));
    }
}
