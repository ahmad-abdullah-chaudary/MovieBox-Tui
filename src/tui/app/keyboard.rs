            ...
            && !self.state.has_active_modal()
        {
            match key.code {
                KeyCode::Char(' ') | KeyCode::Char('p') | KeyCode::Char('P') => {
                    self.action_sender.send(Action::TogglePlayerPause).ok();
                    return None;
                }
                KeyCode::Char('x') | KeyCode::Char('X')
                    if self.state.download_progress.is_none() =>
                {
                    self.action_sender.send(Action::StopPlayback).ok();
                    return None;
                }
                _ => {}
            }
            ...
            if let Some((version, _)) = &self.state.update_available {
                ...
                match key.code {
                    ...
                    KeyCode::Char('o') | KeyCode::Char('O') => {
                        let url = format!(
                            "https://github.com/ahmad-abdullah-chaudary/MovieBox-Tui/releases/tag/v{}",
                            version
                        );
                        let _ = open::that(&url);
                        self.state.update_available = None;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_space_during_playback_toggles_pause_and_skips_search() {
        let mut app = App::new();
        app.state.active_screen = crate::tui::state::Screen::Home;
        app.state.input_mode = InputMode::Normal;
        app.state.is_playing = true;
        app.state.is_resolving_playback = false;
        app.active_player_pid = None;
        ...
        assert!(
            app.state
                .notifications
                .iter()
                .any(|n| n.title == "Pause unavailable"),
            "external/Android playback pause should be reported"
        );
    }

    #[tokio::test]
    async fn test_space_edits_search_when_not_playing() {
        let mut app = App::new();
        app.state.active_screen = crate::tui::state::Screen::Home;
        app.state.input_mode = InputMode::Normal;
        ...
        assert_eq!(app.state.input_mode, InputMode::Editing);
        assert_eq!(app.state.search_query.as_str(), " ");
    }

    #[tokio::test]
    async fn test_quit_while_playing_stops_playback_without_quitting() {
        let mut app = App::new();
        app.state.is_playing = true;
        app.state.is_resolving_playback = false;
        app.active_player_pid = None;

        let first = app.handle_action(Action::Quit).await;
        assert_eq!(first, None, "first q must not quit the app while playing");
        assert!(!app.state.is_playing);

        let second = app.handle_action(Action::Quit).await;
        assert_eq!(second, Some(()), "second q quits the app");
    }

    #[tokio::test]
    async fn test_x_during_playback_sends_stop_playback() {
        let mut app = App::new();
        app.state.active_screen = crate::tui::state::Screen::Home;
        app.state.input_mode = InputMode::Normal;
        app.state.is_playing = true;
        ...
        assert_eq!(
            ...
            "Playback stopped. Press q again to quit the app."
        );
    }
}
