...
fn home_bottom_bar_spans(
    ...
    if state.is_playing {
        let pause_key = if state.basic_terminal { "P" } else { "⏸" };
        let pause_label = if state.player_paused {
            if compact_tabs { "Resume" } else { " Resume" }
        } else if compact_tabs {
            "Pause"
        } else {
            " Pause"
        ...
        if !ultra_compact_tabs {
            bar_spans.push(Span::styled(pause_label, text_style));
        }
        bar_spans.push(Span::raw(sep));
        bar_spans.push(Span::styled("[", bracket_style));
        bar_spans.push(Span::styled("x", shortcut_style));
        bar_spans.push(Span::styled("]", bracket_style));
        if !ultra_compact_tabs {
            bar_spans.push(Span::styled(
                if compact_tabs { "Stop" } else { " Stop" },
                text_style,
            ));
        }
        bar_spans.push(Span::raw(sep));
