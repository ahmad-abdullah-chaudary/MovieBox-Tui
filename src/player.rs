...
pub fn supports_headers(kind: PlayerKind, headers: &[(String, String)]) -> bool {
    ...
    match kind {
        ...
        // VLC cannot attach arbitrary headers to its own requests, but the
        // loopback relay carries them for it, so it is end-to-end capable of
        // playing header-protected streams (see `relay`).
        PlayerKind::Vlc => true,
        PlayerKind::AndroidIntent => {
            android_forwardable_headers(headers)
                && android_opener()
                    .as_ref()
                    .is_some_and(AndroidOpener::forwards_http_headers)
        }
    }
}

/// Whether launching `kind` with `headers` needs the loopback HTTP relay.
///
/// Desktop VLC can only pass `Referer` / `User-Agent` through CLI options;
/// any other header (notably MovieBox's signed CloudFront `Cookie`) must be
/// attached by `crate::player::relay`. All other players carry headers
/// themselves (mpv) or are not supported with them at all (Android intents).
pub fn needs_http_relay(kind: PlayerKind, headers: &[(String, String)]) -> bool {
    kind == PlayerKind::Vlc
...
/// Android external players can only ever be handed Referer / User-Agent header
/// extras (best-effort, not universally honored) through `am`; Cookie and other
/// authentication headers have no Android IPC path at all.
fn android_forwardable_headers(headers: &[(String, String)]) -> bool {
...
impl AndroidOpener {
    /// Whether this opener can attach HTTP header extras (Referer / User-Agent) and
    /// subtitle extras to the outgoing `ACTION_VIEW` intent. `termux-open` and
    /// `termux-open-url` cannot, so header-reliant streams must not be dispatched
    /// through them, or their required authentication would be silently dropped.
    fn forwards_http_headers(&self) -> bool {
        matches!(
            self,
            AndroidOpener::TermuxAm(_) | AndroidOpener::SystemAm(_)
        )
    }

    /// Priority order for intent-capable openers. `termux-am` (or a working `am`) is
    /// preferred because it can set the MIME type, target a specific package, and pass
    /// header/subtitle extras. `termux-open` is a broadcast-based fallback that shows
    /// Android's chooser but cannot carry any of those extras. `termux-open-url` only
    /// opens a URL without a MIME type and is the weakest option.
    const PREFERENCE: &'static [&'static str] = &["termux-am", "termux-open", "termux-open-url"];
}

fn classify_android_executable(path: &str) -> AndroidOpener {
    let normalized = path.replace('\\', "/");
    if normalized.ends_with("termux-am") || normalized.ends_with("/am") || normalized == "am" {
        AndroidOpener::TermuxAm(normalized)
    } else if normalized.ends_with("termux-open-url") {
        AndroidOpener::TermuxOpenUrl(normalized)
    } else {
        AndroidOpener::TermuxOpen(normalized)
    }
}
...
fn probe_android_opener() -> Option<AndroidOpener> {
    ...
    if let Ok(prefix) = std::env::var("PREFIX") {
        for name in AndroidOpener::PREFERENCE {
            let candidate = format!("{prefix}/bin/{name}");
            if Path::new(&candidate).is_file() {
                return Some(classify_android_executable(&candidate));
            }
        }
    }

    for name in AndroidOpener::PREFERENCE {
        let candidate = format!("/data/data/com.termux/files/usr/bin/{name}");
        if Path::new(&candidate).is_file() {
            return Some(classify_android_executable(&candidate));
        }
    }
    ...
    for name in AndroidOpener::PREFERENCE {
        if let Some(path) = find_in_path(name) {
            return Some(classify_android_executable(&path));
        }
    }
...
fn android_intent_command(
    ...
    android_intent_command_with_opener(android_opener().as_ref(), url, subtitle, headers)
}

/// Optional package/component targeting for the `ACTION_VIEW` intent. Only the
/// `am`-based paths can carry it (`-p <package>`); `termux-open-url` accepts it as
/// its second positional argument; `termux-open` cannot target a package at all.
fn android_package_extra() -> Option<String> {
    std::env::var("MOVIEBOX_ANDROID_PACKAGE")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}

fn android_intent_command_with_opener(
    opener: Option<&AndroidOpener>,
    url: &str,
    subtitle: Option<&str>,
    headers: &[(String, String)],
) -> Command {
    let Some(opener) = opener else {
        // Defensive fallback: an opener should have been detected before this
        // point, which is what gates `AndroidIntent` availability in `detect()`.
    ...
    match opener {
        ...
        AndroidOpener::TermuxOpenUrl(path) => {
            let mut cmd = Command::new(path);
            cmd.arg(url);
            if let Some(package) = android_package_extra() {
                cmd.arg(package);
            }
            cmd
        }
...
mod tests {
    ...
    #[test]
    fn header_support_rules_respect_android_opener_capability() {
        let headers = vec![("Cookie".into(), "session=secret".into())];
        assert!(!supports_headers(PlayerKind::AndroidIntent, &headers));
        // VLC is end-to-end header-capable through the loopback relay.
        assert!(supports_headers(PlayerKind::Vlc, &headers));
        // ...but it needs the relay whenever a header beyond Referer/User-Agent
        // appears (that is the Cookie case for MovieBox).
        assert!(needs_http_relay(PlayerKind::Vlc, &headers));
        assert!(needs_http_relay(
            PlayerKind::Vlc,
            &[
                ("referer".into(), "https://example.test/".into()),
        ...
        assert!(!needs_http_relay(PlayerKind::Vlc, &[]));
        assert!(!needs_http_relay(PlayerKind::Mpv, &headers));
        assert!(supports_headers(
            PlayerKind::Vlc,
            &[("referer".into(), "https://example.test/".into())]
        ));
        assert!(supports_headers(PlayerKind::AndroidIntent, &[]));
        // Without an intent-capable opener (termux-am / am), Referer/User-Agent
        // streams must be rejected rather than launched with their headers silently
        // dropped. This is the bare-host contract: tests run with no opener.
        assert!(!supports_headers(
            PlayerKind::AndroidIntent,
            &[
                ("referer".into(), "https://example.test/".into()),
        ...
        // Headers can only be forwarded through `am`-based openers.
        assert!(AndroidOpener::TermuxAm("termux-am".into()).forwards_http_headers());
        assert!(AndroidOpener::SystemAm("/system/bin/am".into()).forwards_http_headers());
        assert!(!AndroidOpener::TermuxOpen("termux-open".into()).forwards_http_headers());
        assert!(!AndroidOpener::TermuxOpenUrl("termux-open-url".into()).forwards_http_headers());

        // Referer/User-Agent are Android-forwardable; Cookie and friends are not.
        assert!(android_forwardable_headers(&[]));
        assert!(android_forwardable_headers(&[
            ("referer".into(), "https://example.test/".into()),
            ("User-Agent".into(), "TestAgent/1.0".into())
        ]));
    ...
    fn test_android_intent_command_structure() {
        // No opener is detected on a non-Termux host, so the defensive fallback
        // must still carry the URL.
    ...
    #[test]
    fn test_android_am_command_carries_package_headers_and_subtitles() {
        let opener = AndroidOpener::TermuxAm("termux-am".into());
        unsafe {
            std::env::set_var("MOVIEBOX_ANDROID_PACKAGE", "org.videolan.vlc");
        }
        let cmd = android_intent_command_with_opener(
            Some(&opener),
            "https://example.test/video.mp4",
            Some("/sdcard/subs/sub.srt"),
            &[
                ("referer".into(), "https://example.test/".into()),
                ("user-agent".into(), "TestAgent/1.0".into()),
            ],
        ...
        unsafe {
            std::env::remove_var("MOVIEBOX_ANDROID_PACKAGE");
        }
        let args = cmd
            .get_args()
            .map(|a| a.to_string_lossy().into_owned())
        ...
        assert!(args.contains(&"org.videolan.vlc".to_string()));
        assert!(args.contains(&"subtitles_location".to_string()));
        assert!(args.contains(&"subs".to_string()));
    ...
    #[test]
    fn test_android_termux_open_url_takes_package_as_second_argument() {
        let opener = AndroidOpener::TermuxOpenUrl("termux-open-url".into());
        unsafe {
            std::env::set_var("MOVIEBOX_ANDROID_PACKAGE", "is.xyz.mpv");
        }
        let cmd = android_intent_command_with_opener(
            Some(&opener),
            "https://example.test/video.mp4",
